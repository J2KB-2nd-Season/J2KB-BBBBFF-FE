...
function ProductDetailPage(props) {
    const num = props.match.params.product_num;
    ...
    useEffect(() => {
        setProduct(products.find((product) => product.num === parseInt(num)));
    }, [num]);
    /* 나중에는 서버에서 가져옴 */
    ...
    const changeImg = (event) => {
        ...
        } else document.querySelector(`.${styles.bigImg}`).src = event.target.src;
    };
            ...
            <div>
                ...
                <div className={styles.container}>
                    ...
                    <div className={styles.product}>
                        ...
                        <div>
                            <Descriptions bordered size="small" title="Product Info 제품정보">
                                <Descriptions.Item label="상품명" span={3}>
                                    {Product.name}
                                </Descriptions.Item>
                                <Descriptions.Item label="상품번호" span={3}>
                                    {Product.num}
                                </Descriptions.Item>
                                <Descriptions.Item label="재고" span={3}>
                                    {Product.stock}
                                </Descriptions.Item>
                                <Descriptions.Item label="상품설명" span={3}>
                                    {Product.detail}
                                </Descriptions.Item>
                            </Descriptions>
                            <br />
                            <Descriptions bordered title="Price Info 가격정보">
                                <Descriptions.Item label="판매가" span={3}>
                                    {Product.price}원
                                </Descriptions.Item>
                            </Descriptions>
